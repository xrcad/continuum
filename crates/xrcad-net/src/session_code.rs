...
pub fn decode(code: &str) -> Result<(SessionId, SocketAddr), NetError> {
    ...
    if bytes.len() < cursor + 16 {
        return Err(NetError::SessionCode(
            "truncated: missing session ID".into(),
        ));
    }
    ...
    if bytes.len() < cursor + 1 {
        return Err(NetError::SessionCode(
            "truncated: missing address flag".into(),
        ));
    }
    ...
    let ip: IpAddr = match flag {
        FLAG_IPV4 => {
            if bytes.len() < cursor + 4 {
                return Err(NetError::SessionCode(
                    "truncated: missing IPv4 address".into(),
                ));
            }
        ...
        FLAG_IPV6 => {
            if bytes.len() < cursor + 16 {
                return Err(NetError::SessionCode(
                    "truncated: missing IPv6 address".into(),
                ));
            }
    ...
    if bytes.len() < cursor + 1 {
        return Err(NetError::SessionCode(
            "truncated: missing version byte".into(),
        ));
    }
